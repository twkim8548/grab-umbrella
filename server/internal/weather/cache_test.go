...
func TestForecastCacheHit(t *testing.T) {
	...
	if _, ok := c.get(60, 127, "20260623", "0800"); ok {
		t.Fatal("expected miss before put")
	}
	c.put(60, 127, "20260623", "0800", items)

	got, ok := c.get(60, 127, "20260623", "0800")
	if !ok {
	...
	// 다른 격자는 미스.
	if _, ok := c.get(61, 127, "20260623", "0800"); ok {
		t.Error("expected miss for different grid")
	}
	// 다른 발표본은 미스.
	if _, ok := c.get(60, 127, "20260623", "1100"); ok {
...
func TestForecastCacheEvictsOldRelease(t *testing.T) {
	c := newForecastCache()
	c.put(60, 127, "20260623", "0500", []FcstItem{{FcstValue: "old"}})
	c.put(60, 127, "20260623", "0800", []FcstItem{{FcstValue: "new"}})

	// 같은 격자의 이전 발표본(0500)은 정리되어야 한다.
	if _, ok := c.get(60, 127, "20260623", "0500"); ok {
		t.Error("expected old release evicted")
	}
	// 최신 발표본(0800)은 유지.
	if got, ok := c.get(60, 127, "20260623", "0800"); !ok || got[0].FcstValue != "new" {
		t.Errorf("expected latest release retained, got ok=%v %+v", ok, got)
	}

	// 다른 격자는 영향 없음.
	c.put(99, 99, "20260623", "0500", []FcstItem{{FcstValue: "other"}})
	c.put(60, 127, "20260623", "1100", []FcstItem{{FcstValue: "newer"}})
	if _, ok := c.get(99, 99, "20260623", "0500"); !ok {
...
func TestForecastCacheConcurrent(t *testing.T) {
	...
	for i := 0; i < goroutines; i++ {
		go func(i int) {
			...
			c.put(60, 127, "20260623", "0800", []FcstItem{{FcstValue: "v"}})
			_, _ = c.get(60, 127, "20260623", "0800")
			// 격자 다양화로 eviction 경로도 동시에 친다.
			c.put(i, i, "20260623", "0800", []FcstItem{{FcstValue: "g"}})
		}(i)
	}
	wg.Wait()
