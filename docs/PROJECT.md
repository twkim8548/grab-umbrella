...
- **홈 화면의 하루 우산 결론은 한 번에 결정한다.** 출근 OR 퇴근, 하나라도 비 → "우산 챙기세요". 푸시는 출근·퇴근 슬롯별로 판단한다. 자세한 화면 모델은 [design-main-screen.md](./design-main-screen.md).
...
| 호스팅 | AWS Lambda Function URL | SAM 컨테이너 이미지, Web Adapter로 API 공개 |
| 스케줄링 | EventBridge Scheduler | Lambda cron을 10분마다 직접 호출 |
...
핵심 기능과 운영 배포를 완료했다. AWS에서 API Lambda와 10분 주기 cron Lambda가 동작하며, 앱의 초기 설정·알림 설정 서버 동기화·예보 캐시와 백그라운드 갱신·오류 재시도 흐름을 iPhone Release 빌드로 검증했다. 기존 실기기 푸시 파이프라인도 전 구간(토큰→Expo→APNs→단말, cron 매칭→발송→중복방지)을 검증했다.

현재 빌드는 개발용 서명으로 기기에 직접 설치한 검증본이다.

자세한 구현 상태는 [DECISIONS.md](./DECISIONS.md#구현-상태).
